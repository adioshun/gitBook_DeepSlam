...
We are givena set of measurements $$Z = \{z_k : k = 1, ...,m\} $$ such that each measurement can be expressed as a function of X , i.e.$$z_k = h_k(X_k)+\epsiulon_k$$, where $$X_k \subseteq X$$ is a subset of the variables,hk(·) is a known function (the measurement or observationmodel), and k is random measurement noise.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA5NTcwNjcwXX0=
-->