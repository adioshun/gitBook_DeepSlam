...
- 초창기 연구는 [176]이다. 이 방식은 2D 레이저 스캐너를 이용하여 geometric map 을 생성하고 associative Markov network으로 식별된 semantic places를 Fuse하였다. An early work was that of Mozos et al. [176], which builds a geometric map using a 2D laser scan and then fuses the classified semantic places from each robot pose through an associative Markov network in an offline manner. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc3NTU5OTc1OF19
-->